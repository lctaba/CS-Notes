...
目前数据链路层广泛使用了循环冗余检验（CRC）来检查比特差错。其实就是二进制除法的余数。

[循环冗余校验](https://baike.baidu.com/item/循环冗余校验/3219009?fromModule=lemma_inlink)（CRC，Cyclhc Redundancy Check）是利用除法及余数的原理来作错误侦测（Error Detecting）的。它将要发送的数据比特序列当作一个多项式m（x）的系数，发送时用双方预先约定的生成多项式G（x）去除，求得一个余数多项式，将余数多项式加到数据多项式之后合成A（x）多项式发送到接收端，接收端同样用G（x）去除接收到的数据，然后把计算结果和实际接收到的余数多项式数据进行比较，相同则表示传输正确。[CRC校验](https://baike.baidu.com/item/CRC校验/3439037?fromModule=lemma_inlink)检错能力强，容易实现，是应用最广的检错码编码方式之一。
