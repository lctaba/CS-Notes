...
[Comet技术详解：基于HTTP长连接的Web端实时通信技术-阿里云开发者社区 (aliyun.com)](https://developer.aliyun.com/article/633717)
...
[SSL协议原理详解-CSDN博客](https://blog.csdn.net/qq_38265137/article/details/90112705)

[SSL加密原理 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/336146921)

### 中间人攻击

[【网络安全】——中间人攻击-CSDN博客](https://blog.csdn.net/A1_3_9_7/article/details/135485598)

- WIFI劫持通信流量

- APR欺骗：攻击者伪装网关返回伪造的mac地址，伪造的地址会被用户加入APR缓存
- DNS欺骗：攻击者篡改域名对应的IP地址，达到重定向用户访问的目的。
- SSL劫持：攻击者伪造服务器的证书和公钥与客户端通信。甚至窃取真实证书。

## 七、HTTP/2.0

[计网—HTTP1.0、HTTP2.0、HTTP3.0优缺点、区别及联系_http1.0 http2.0 http3.0 有什么不同-CSDN博客](https://blog.csdn.net/weixin_60630451/article/details/131525477)

[一文读懂 HTTP/1、HTTP/2、HTTP/3-51CTO.COM](https://www.51cto.com/article/612101.html)

[HTTP/1的缺点总结 - 不挑食的程序员 - SegmentFault 思否](https://segmentfault.com/a/1190000013519925?utm_source=tag-newest)
...
HTTP/2 实现服务器推送（Server Push）的基本原理是在客户端请求某个资源时，服务器可以主动将其他相关资源推送给客户端，以提高性能和加载速度。这里简要介绍一下 HTTP/2 中服务器推送的实现方式：

1. **连接复用**：HTTP/2 使用单一的 TCP 连接来处理多个并行的请求和响应，这消除了每个请求都要建立新连接的开销。因此，在这种情况下，服务器可以更有效地推送资源，因为连接已经建立好了。
2. **头部压缩**：HTTP/2 使用了头部压缩技术，将头部信息压缩后发送，减少了数据传输的大小。这使得服务器可以更快地将推送资源的头部信息发送给客户端。
3. **优先级**：HTTP/2 支持请求优先级，客户端可以通过设置请求的优先级来告诉服务器哪些资源更重要。服务器在推送资源时可以考虑这些优先级，确保推送的资源是客户端最需要的。
4. **推送流**：HTTP/2 中引入了推送流（Push Stream），服务器可以在客户端请求一个资源时主动发起推送流，将相关资源推送给客户端。客户端接收到推送流后可以决定是否接受推送的资源。

实现服务器推的技术：

- 段轮询：客户端不断发送请求，服务端不管有没有数据都返回。
- 长轮询：客户端发送请求，服务端等有数据了再返回。
- 长连接：连接一直建立，客户端可以用回调函数处理服务端推送的请求。
- webSocket：自定义轻量级socket实现全双工通信。
